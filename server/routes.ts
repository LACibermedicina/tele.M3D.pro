...
Notas clínicas: ${session.aiClinicalSummary || 'Nenhuma nota registrada'}
...
Retorne apenas o JSON válido.`;
      ...
      const clinicalAsset = await storage.createClinicalAsset({
        ...
        aiAnalysis: aiAnalysis.summary || 'Análise pendente',
        timeline: new Date()
      });
...
Pressão arterial: 120/80 mmHg, frequência cardíaca: 78 bpm.
  ...
  app.post('/api/prescriptions', requireAuth, async (req, res) => {
    try {
      ...
      try {
        await storage.processDebit(req.user.id, 5, 'Criação de prescrição', 'prescription_creation', undefined, undefined, undefined);
      } catch (tmcError) {
...
- Idade: ${patientInfo.dateOfBirth ? new Date().getFullYear() - new Date(patientInfo.dateOfBirth).getFullYear() : 'Não especificado'} anos
- Gênero: ${patientInfo.gender || 'Não especificado'}
- Peso: Não especificado kg
...
REGRAS IMPORTANTES:
1. REFERÊNCIAS MÉDICAS: Se houver referências médicas fornecidas pelo médico (PDFs), use-as como fonte PRIORITÁRIA. Baseie suas respostas PRIMEIRO nas referências disponíveis.
2. SEM REFERÊNCIAS: Se NÃO houver referências disponíveis ou a resposta não estiver nelas, valide suas respostas em fontes médicas de alto nível de evidência (Cochrane, PubMed, UpToDate, guidelines nacionais/internacionais). CITE SEMPRE a fonte utilizada.
3. OBJETIVIDADE: Mantenha respostas objetivas e diretas, aproximadamente 30 palavras, exceto quando análises mais detalhadas forem solicitadas.
4. NÃO REPETIR: Evite repetir informações já ditas no histórico da conversa. Sempre traga informação nova ou complementar.
5. PRECISÃO: Sempre cite as fontes quando disponíveis e seja preciso e técnico nas respostas.
6. TRANSPARÊNCIA: Indique claramente se a resposta vem das referências do médico ou de fontes médicas baseadas em evidências.`;
      } else {
...
REGRAS IMPORTANTES:
1. REFERÊNCIAS MÉDICAS: Se houver referências médicas fornecidas pelo médico (PDFs), baseie suas respostas PRIORITARIAMENTE nelas.
2. SEM REFERÊNCIAS: Se NÃO houver referências ou a resposta não estiver nelas, valide suas respostas em fontes médicas confiáveis e baseadas em evidências atuais. CITE SEMPRE a fonte (ex: "Segundo a OMS...", "De acordo com as Diretrizes do Ministério da Saúde...").
...
4. NÃO REPETIR: Evite repetir informações já ditas anteriormente na conversa. Sempre traga algo novo.
5. NÃO DIAGNOSTICAR: Você NÃO faz diagnósticos. Sempre recomende consulta médica para avaliação adequada.
6. EMERGÊNCIAS: Em casos de emergência, oriente a procurar atendimento imediato (UPA, Pronto Socorro, SAMU 192).
7. TRANSPARÊNCIA: Indique se a informação vem das referências do médico ou de fontes médicas baseadas em evidências.`;
      }
...
1. 📖 PRIORIDADE ABSOLUTA - REFERÊNCIAS MÉDICAS:
   • Se referências médicas foram fornecidas pelo profissional, use PRIORITARIAMENTE essas informações
   • SEMPRE cite a fonte quando usar informação de uma referência
   • Se a informação NÃO estiver nas referências, valide sua resposta em fontes médicas confiáveis e baseadas em evidências (OMS, Ministério da Saúde, Cochrane, PubMed)
   • CITE SEMPRE a fonte utilizada (ex: "Segundo a OMS...", "De acordo com o Ministério da Saúde...")
   • Prefira dados das referências sobre conhecimento geral
   • Seja transparente: informe se a resposta vem das referências do médico ou de fontes médicas baseadas em evidências
